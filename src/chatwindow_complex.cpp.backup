...
void ChatWindow::setupUI() {
    ...
    // Create horizontal splitter for peer list and chat
    QSplitter* splitter = new QSplitter(Qt::Horizontal, this);
    splitter->setStyleSheet("QSplitter::handle { background-color: #202C33; width: 2px; }");

    // Peer list on the left
    QWidget* peerContainer = new QWidget(this);
    peerContainer->setMaximumWidth(200);
    auto* peerLayout = new QVBoxLayout(peerContainer);

    QLabel* peerLabel = new QLabel("Active Peers", this);
    peerLabel->setStyleSheet(
        "font-weight: bold; "
        "color: #8696A0; "
        "padding: 8px;"
    );
    ...
    splitter->addWidget(peerContainer);
...
void ChatWindow::updatePeerList(const QList<QString>& peers) {
    ...
    for (const QString& peer : peers) {
        if (peer != currentNodeId) {
            ...
            // Update peer list widget
            bool found = false;
            for (int i = 0; i < peerListWidget->count(); ++i) {
                if (peerListWidget->item(i)->text().contains(peer)) {
                    found = true;
                    break;
                }
            }
            ...
            if (!found) {
                QString status = peerStatus.value(peer, true) ? "Online" : "Offline";
                peerListWidget->addItem(QString("%1 (%2)").arg(peer).arg(status));
            }
        }
    }
...
void ChatWindow::updatePeerStatus(const QString& peerId, bool active) {
    ...
    // Update peer list widget
    for (int i = 0; i < peerListWidget->count(); ++i) {
        QListWidgetItem* item = peerListWidget->item(i);
        if (item->text().startsWith(peerId)) {
            QString status = active ? "Online" : "Offline";
            item->setText(QString("%1 (%2)").arg(peerId).arg(status));
